...
const CONFIG_WEB_URLS = {
    ...
    prod: {
        home: {
            ...
            // 个人紧急情况上报
            个人紧急情况上报:
                "/zql-wx-mobile/report-process/report-type?taskType=apply",
            // 个人紧急情况上报审批
            个人紧急情况上报审批:
                "/zql-wx-mobile/report-process?taskType=audit",
            // 进场申请
...
function getWebUrl(moduleName, type, param) {
    ...
    if (!configData) {
        console.warn(
            "外链所在的页面配置不存在，请前往 /pages/webview/config_url 查看一下"
        );
        return;
    }
    if (!configData[type]) {
        console.warn(
            "外链类型不存在，请前往 /pages/webview/config_url 查看一下"
        );
        return;
    }
